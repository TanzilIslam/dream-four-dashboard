    ...
    user.role === "admin"
      ? await sql`
          SELECT c.*,
                 ...
                 COALESCE((SELECT COUNT(*) FROM orders o WHERE o.customer_id = c.id AND o.status IN ('delivered', 'paid')), 0)::int AS total_orders,
                 COALESCE((SELECT SUM(o.quantity) FROM orders o WHERE o.customer_id = c.id AND o.status IN ('delivered', 'paid')), 0)::int AS total_quantity,
