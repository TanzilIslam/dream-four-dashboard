...
function PaymentConfigInner() {
  ...
  const dueAllowed = form.watch("due_allowed");
    ...
    fetch("/api/settings/payment-config")
      ...
      .then((c) => {
        form.reset({
          ...
          late_punch_threshold: typeof c.late_punch_threshold === "string"
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Global business rules applied across all partners.</p>
      </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Can customers carry an outstanding balance?</p>
                </div>
                <Switch checked={dueAllowed} onCheckedChange={(v) => form.setValue("due_allowed", v)} />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {form.formState.errors.max_due_per_customer && (
                  <p className="text-xs text-destructive">{form.formState.errors.max_due_per_customer.message}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">A partner is late if they punch in after this time.</p>
                {form.formState.errors.late_punch_threshold && (
                  <p className="text-xs text-destructive">{form.formState.errors.late_punch_threshold.message}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">Fallback alert threshold for products without their own.</p>
                {form.formState.errors.low_stock_default && (
                  <p className="text-xs text-destructive">{form.formState.errors.low_stock_default.message}</p>
                )}
              </div>
